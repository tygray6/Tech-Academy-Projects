...
namespace Bewander.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // POST: Record in UserProfile table 
        ...
        {
            Console.WriteLine("In the ExternalLoginCallback, Facebook detected.");
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    result = await UserManager.AddLoginAsync(ASPuser.Id, info.Login);
                    if (result.Succeeded)
                    {
                        ...
                        User user = new User();
                        user.UserID = ASPuser.Id;
                        //TODO: Get first and last name from facebook account
                        user.FirstName = model.FirstName;
                        user.LastName = model.LastName;
                        user.Permission = 0;
                        user.DateJoined = DateTime.Now.ToString();
